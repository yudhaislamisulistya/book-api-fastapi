...
router = APIRouter(
    prefix="/books",
    tags=["books"],
    responses={404: {"description": "Not found"}, "500": {"description": "Internal Server Error"}, "400": {"description": "Bad Request"}, "401": {"description": "Unauthorized"}, "403": {"description": "Forbidden"}, "405": {"description": "Method Not Allowed"}, "409": {"description": "Conflict"}, "422": {"description": "Unprocessable Entity"}, "429": {"description": "Too Many Requests"}, "503": {"description": "Service Unavailable"}}
)
...
@router.get("/")
async def read_books(db: Session = Depends(get_db), skip: int = 0, limit: int = 100, response: Response = None):
...
async def create_book(book: schema_book.Book, db: Session = Depends(get_db), response: Response = None):
    results = book_controller.create_book(db=db, book=book)
    if not results:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail={
            "message": "Book already exists", "status_code": status.HTTP_400_BAD_REQUEST
        })
    response_content = {
...
async def delete_book(book_id: int, db: Session = Depends(get_db), response: Response = None):
